...
export default function StudentsPage() {
  ...
  // 当前页可见的学生 ID 集合
  const visibleStudentIds = useMemo(() => new Set(filteredStudents.map(s => s.id)), [filteredStudents])

  const selectedStudents = useMemo(
    () => filteredStudents.filter(s => selectedIds.has(s.id)),
    [filteredStudents, selectedIds]
  )
  const allVisibleSelected = visibleStudentIds.size > 0 && selectedStudents.length === visibleStudentIds.size
  ...
  function toggleSelectAll() {
    setSelectedIds(prev => {
      ...
      if (allVisibleSelected) {
        for (const id of visibleStudentIds) next.delete(id)
      } else {
  ...
  const handleBatchDelete = async () => {
    ...
    await studentApi.batchDeleteStudents(selected.map(s => s.id))
    await loadData()
  }
  ...
  return (
    <div className="h-full overflow-auto">
      ...
      {/* 批量添加弹窗 */}
      <Modal open={showBatchAdd} onClose={() => { setShowBatchAdd(false); setBatchNames('') }} title="批量添加学生">
        <textarea
      ...
      {/* 添加学生弹窗 */}
      <Modal open={showAdd} onClose={() => { setShowAdd(false); setAddName('') }} title="添加学生" width="sm">
        <input
          type="text"
          placeholder="学生姓名"
          value={addName}
      ...
      {/* 添加小组弹窗 */}
      <Modal open={showAddGroup} onClose={() => setShowAddGroup(false)} title="添加小组" width="sm">
        <input
          type="text"
          placeholder="小组名称"
          value={addGroupName}
      ...
      {/* 编辑小组弹窗 */}
      <Modal open={editGroup !== null} onClose={() => setEditGroup(null)} title="编辑小组" width="sm">
        <input
          type="text"
          value={editGroupName}
