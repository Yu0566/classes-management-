          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                tab === 'deductions' ? 'bg-red-500 text-white' : 'border text-stone-600 hover:bg-stone-50'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                tab === 'manual' ? 'bg-blue-500 text-white' : 'border text-stone-600 hover:bg-stone-50'
              }`}
            >
          ...
          {tab === 'deductions' ? (
            <table className="w-full">
              <thead>
                <tr className="bg-stone-50 border-b">
                  ...
                  <th className="text-left px-4 py-3 text-sm font-medium text-stone-500">扣分</th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-stone-500">原因</th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-stone-500">日期</th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-stone-500">时间</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                ...
                {filteredDeductions.length === 0 && (
                  <tr><td colSpan={5} className="text-center py-12 text-stone-400 text-sm">暂无扣分记录</td></tr>
                )}
              </tbody>
            </table>
          ) : (
            <table className="w-full">
              <thead>
                <tr className="bg-stone-50 border-b">
                  <th className="text-left px-4 py-3 text-sm font-medium text-stone-500">姓名</th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-stone-500">调整</th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-stone-500">原因</th>
                  <th className="text-left px-4 py-3 text-sm font-medium text-stone-500">时间</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                ...
                {filteredManual.length === 0 && (
                  <tr><td colSpan={4} className="text-center py-12 text-stone-400 text-sm">暂无手动调整记录</td></tr>
                )}
              </tbody>
            </table>
          )}
